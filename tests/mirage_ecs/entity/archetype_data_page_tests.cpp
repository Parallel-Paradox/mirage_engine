...
//   EXPECT_EQ(page.capacity(), page.buffer().size() /
//   page.descriptor()->size()); EXPECT_EQ(page.size(), 0);
// }
...
TEST_F(ArchetypeDataPageTests, Iterator) {
  const bool x = std::bidirectional_iterator<ArchetypeDataPage::ConstIterator>;
  EXPECT_TRUE(x);
}
